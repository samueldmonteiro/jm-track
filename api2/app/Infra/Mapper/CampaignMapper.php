...
class CampaignMapper
{
    public static function eloquentToCampaign(CampaignModel $model, array $relations = []): Campaign
    {
        ...
        return new Campaign(
            ...
            $model->start_date,
            $model->end_date,
            in_array('trafficExpenses', $relations)  ? TrafficExpenseMapper::eloquentCollectionToEntities($model->trafficExpenses, ['trafficSource']) : [],
            in_array('trafficReturns', $relations)  ? TrafficReturnMapper::eloquentCollectionToEntities($model->trafficReturns, ['trafficSource']) : [],
            in_array('campaignMetrics', $relations)  ? CampaignMetricMapper::eloquentCollectionToEntities($model->campaignMetrics) : [],
        );
    }
